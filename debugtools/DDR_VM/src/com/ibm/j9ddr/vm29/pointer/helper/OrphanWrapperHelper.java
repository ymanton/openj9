...
public class OrphanWrapperHelper {
	public static J9ROMClassPointer romClass(OrphanWrapperPointer ptr, U8Pointer[] cacheHeader) throws CorruptDataException {
		if (null == cacheHeader) {
			return J9ROMClassPointer.cast(U8Pointer.cast(ptr).add(ptr.romClassOffset()));
		} else {
			if (J9ShrOffSetPointer.cast(ptr.romClassOffsetEA()).offset().eq(0)) {
				return J9ROMClassPointer.cast(U8Pointer.NULL);
			}
			return J9ROMClassPointer.cast(cacheHeader[0].add(J9ShrOffSetPointer.cast(ptr.romClassOffsetEA()).offset()));
		}
	}
}
