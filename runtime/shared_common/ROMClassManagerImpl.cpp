...
SH_ROMClassManagerImpl::findNextExisting(J9VMThread* currentThread, void * &findNextIterator, void * &firstFound, U_16 classnameLength, const char* classnameData)
{
	...
	if (TYPE_ORPHAN == ITEMTYPE(currentItem)) {
		...
		returnVal = (J9ROMClass*) _cache->getAddressFromJ9ShrOffset(((OrphanWrapper*) ITEMDATA(currentItem))->romClassOffset);
	} else {
		Trc_SHR_RMI_findNextROMClass_FoundClass_Event(currentThread);
		returnVal = (J9ROMClass*) _cache->getAddressFromJ9ShrOffset(((ROMClassWrapper*) ITEMDATA(currentItem))->romClassOffset);
	}
	...
	if (prev != NULL) {
		...
		if (TYPE_ORPHAN == ITEMTYPE(prevItem)) {
			prevVal = (J9ROMClass*) _cache->getAddressFromJ9ShrOffset(((OrphanWrapper*) ITEMDATA(prevItem))->romClassOffset);
		} else {
			prevVal = (J9ROMClass*) _cache->getAddressFromJ9ShrOffset(((ROMClassWrapper*) ITEMDATA(prevItem))->romClassOffset);
		}
