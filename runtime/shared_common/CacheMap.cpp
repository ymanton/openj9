...
SH_CacheMap::initialize(J9JavaVM* vm, J9SharedClassConfig* sharedClassConfig, BlockPtr memForConstructor, const char* cacheName, I_32 cacheTypeRequired, I_8 topLayer, bool startupForStats)
{
	...
	for (UDATA i = 0; i <= J9SH_LAYER_NUM_MAX_VALUE; i++) {
		_cacheAddressArray[i].cacheHeader = NULL;
		_cacheAddressArray[i].cacheEnd = NULL;
	}
	_numOfCacheLayers = 0;
		...
		do {
			...
			if (rc == CC_STARTUP_OK) {
				...
				if (ccToUse->enterWriteMutex(currentThread, false, fnName) == 0) {
					...
					if ((*runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION) && !isReadOnly) {
...
error:
	...
	do {
		...
		if (ccToUse->enterWriteMutex(currentThread, false, fnName) == 0) {
			 ...
			 */
			if ((0 == (*runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_NESTED))
				&& (!isReadOnly)
			) {
...
SH_CacheMap::handleStartupError(J9VMThread* currentThread, SH_CompositeCacheImpl* ccToUse, IDATA errorCode, U_64 runtimeFlags, UDATA verboseFlags, bool *doRetry)
{
	...
	if (0 == (runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_STATS) && (false == ccToUse->isRunningReadOnly()) && J9_ARE_NO_BITS_SET(runtimeFlags, J9SHR_RUNTIMEFLAG_FAKE_CORRUPTION)) {
...
SH_CacheMap::initializeROMSegmentList(J9VMThread* currentThread)
{
	...
	/* TODO: This makes no sense for cachelets */
	if (!(config->metadataMemorySegment = createNewSegment(currentThread, MEMORY_TYPE_SHARED_META, vm->memorySegments, cacheBase, (U_8*)_ccHead->getMetaAllocPtr(), cacheDebugAreaStart, cacheDebugAreaStart))) {
		result = 0;
	}
...
#if defined(MULTI_LAYER_CACHE)
			srcw.modContextOffset.cacheLayer = 0;
#endif /* defined(MULTI_LAYER_CACHE) */
...
SH_CacheMap::markStale(J9VMThread* currentThread, ClasspathEntryItem* cpei, bool hasWriteMutex) 
{
	...
	while (retryCount < MARK_STALE_RETRY_TIMES) {
		if (hasWriteMutex || (_ccHead->enterWriteMutex(currentThread, true,fnName)==0)) {	/* true = lockCache */
			...
			_cc->startCriticalUpdate(currentThread);		/* Un-protects cache header */
			_cc->findStart(currentThread);

			cpei->flags |= MARKED_STALE_FLAG;
			localCPM->markClasspathsStale(currentThread, cpei);

			do {
				it = (ShcItem*)_cc->nextEntry(currentThread, &unused);		/* should ignore stale items */
...
SH_CacheMap::reportCorruptCache(J9VMThread* currentThread, SH_CompositeCacheImpl* ccToUse)
{
	...
	if (hasRefreshMutex || enteredRefreshMutex) {
		if (!_cacheCorruptReported) {
			...
			if ((*_runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION) && (false == ccToUse->isRunningReadOnly())) {
...
/**
 *  Set the _cacheAddressArray
 */
void
SH_CacheMap::setCacheAddressArray(void)
{
	_numOfCacheLayers = 0;
	SH_CompositeCacheImpl* ccToUse = _ccTail;
	do {
		Trc_SHR_Assert_True(_numOfCacheLayers <= J9SH_LAYER_NUM_MAX_VALUE);
		_cacheAddressArray[_numOfCacheLayers].cacheHeader = (void*)ccToUse->getCacheHeaderAddress();
		_cacheAddressArray[_numOfCacheLayers].cacheEnd = (void*)ccToUse->getCacheEndAddress();
		ccToUse = ccToUse->getPrevious();
		_numOfCacheLayers += 1;
	} while (NULL != ccToUse);
	_numOfCacheLayers -= 1;
...
/**
 *	Helper method to turn an address into J9ShrOffSet using _cacheAddressArray
 *
...
SH_CacheMap::getJ9ShrOffsetFromAddress(const void* address, J9ShrOffSet* offSet)
{
	if (UnitTest::OSCACHE_TEST == UnitTest::unitTest || UnitTest::SHAREDCACHE_API_TEST == UnitTest::unitTest) {
		if (NULL == _cacheAddressArray[0].cacheHeader) {
			setCacheAddressArray();
		}
	}
	for (U_32 layer = 0; layer <= _numOfCacheLayers; layer++) {
		if ((_cacheAddressArray[layer].cacheHeader < address)
			&& (address < _cacheAddressArray[layer].cacheEnd)
		) {
#if defined(MULTI_LAYER_CACHE)
			offSet->cacheLayer = layer;
#endif /* defined(MULTI_LAYER_CACHE) */
			offSet->offset = (U_32)((U_8*)address - (U_8*)_cacheAddressArray[layer].cacheHeader);
			return;
		}
	}
...
/**
 *	Helper method to turn a J9ShrOffSet into an address in using _cacheAddressArray
 *
 *	@param [in] offSet A J9ShrOffSet.
 *
 *	@return The corresponding address from J9ShrOffSet.
 */
void*
SH_CacheMap::getAddressFromJ9ShrOffset(J9ShrOffSet offSet)
{
...
#if defined(MULTI_LAYER_CACHE)
	return (U_8*)_cacheAddressArray[offSet.cacheLayer].cacheHeader + offSet.offset;
#else	/* defined(MULTI_LAYER_CACHE) */
	return (U_8*)_cacheAddressArray[0].cacheHeader + offSet.offset;
#endif /* defined(MULTI_LAYER_CACHE) */
}
