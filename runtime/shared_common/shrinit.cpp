...
j9shr_init(J9JavaVM *vm, UDATA loadFlags, UDATA* nonfatal)
{
	...
	if (-1 == layer) {
		/* "layer=" is not used in the command line */
		if (-1 == maxLayer) {
...
findExistingCacheLayerNumbers(J9JavaVM* vm, const char* ctrlDirName, const char* cacheName, U_64 runtimeFlags, I_8 *maxLayerNo)
{
	...
	for (I_8 l = 0; l <= J9SH_LAYER_NUM_MAX_VALUE; l++) {
		if (1 == j9shr_stat_cache(vm, ctrlDirName, 0 , cacheName, &versionData, OSCACHE_CURRENT_CACHE_GEN, l)) {
			if (l > maxLayer) {
				maxLayer = l;
			}
		}
