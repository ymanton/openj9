...
// NOTE: entries here must match the entries in enum AOTIneligibilityReason
static const char *AOTIneligibilityReasonNames[] = {
    "",
    "jniNative",
    "newInstanceThunk",
    "methodHandleThunk",
    ...
    "jitserver instructed",
};

static_assert(sizeof(AOTIneligibilityReasonNames) / sizeof(AOTIneligibilityReasonNames[0]) == AOT_INELIGIBLE_LAST_ENTRY,
    "AOTIneligibilityReasonNames must match AOTIneligibilityReason enum");
 ...
 * @param ineligibleForRelocatableCompile Reference to a AOTIneligibilityReason; Indicates the reason the current
 * compilation is ineligible for AOT compilation, or RELOCATABLE_COMPILE_OK if eligible. (output)
    ...
    AOTIneligibilityReason &ineligibleForRelocatableCompile, TR_RelocationRuntime *reloRuntime)
{
    ...
    if (!entry->isAotLoad()) // We don't/can't relocate this method
    {
        ...
        if (entry->isOutOfProcessCompReq()) {
            ...
            // ineligibleForRelocatableCompile should be true at the server side if the client
            // requests AOT.
            ineligibleForRelocatableCompile
                = entry->_useAotCompilation ? RELOCATABLE_COMPILE_OK : AOT_INELIGIBLE_JITSERVER_INSTRUCTED;
#endif
        } else {
            TR::IlGeneratorMethodDetails &details = entry->getMethodDetails();
            // Test for this method's suitability for a relocatable compile, ignoring any
            // local SCC criteria.
            AOTIneligibilityReason ineligibilityReasonForRelocatableCompileDisregardingSCC = RELOCATABLE_COMPILE_OK;

            // Unsupported compilations
            if (entry->isJNINative())
                ineligibilityReasonForRelocatableCompileDisregardingSCC = AOT_INELIGIBLE_JNI;
            else if (details.isNewInstanceThunk())
                ineligibilityReasonForRelocatableCompileDisregardingSCC = AOT_INELIGIBLE_NEW_INSTANCE_THUNK;
            else if (details.isMethodHandleThunk())
                ineligibilityReasonForRelocatableCompileDisregardingSCC = AOT_INELIGIBLE_METHOD_HANDLE_THUNK;
            else if (entry->isDLTCompile())
                ineligibilityReasonForRelocatableCompileDisregardingSCC = AOT_INELIGIBLE_DLT;
            // Only generate AOT compilations for first time compiles
            else if (TR::CompilationInfo::isCompiled(method))
                ineligibilityReasonForRelocatableCompileDisregardingSCC = AOT_INELIGIBLE_RECOMP;
            // See eclipse-openj9/openj9#11879 for details
            else if (TR::Options::getCmdLineOptions()->getOption(TR_FullSpeedDebug) && entry->_oldStartPC)
                ineligibilityReasonForRelocatableCompileDisregardingSCC = AOT_INELIGIBLE_FSD;
            // Eligibility checks
            else if (_compInfo.isMethodIneligibleForAot(method, &trMemory, fe))
                ineligibilityReasonForRelocatableCompileDisregardingSCC = AOT_INELIGIBLE_METHOD;
            else if (TR::Options::getAOTCmdLineOptions()->getOption(TR_AOTCompileOnlyFromBootstrap)
                && !fe->isClassLibraryMethod((TR_OpaqueMethodBlock *)method))
                ineligibilityReasonForRelocatableCompileDisregardingSCC = AOT_INELIGIBLE_BOOTSTRAP_ONLY;
            ...
            else if (entry->_doNotAOTCompile)
                ineligibilityReasonForRelocatableCompileDisregardingSCC = AOT_INELIGIBLE_PREV_FAIL;
            ...
            else if (_jitConfig->inlineFieldWatches)
                ineligibilityReasonForRelocatableCompileDisregardingSCC = AOT_INELIGIBLE_FIELD_WATCH;

            AOTIneligibilityReason ineligibilityReasonForRelocatableCompile
                = ineligibilityReasonForRelocatableCompileDisregardingSCC;
            if (ineligibilityReasonForRelocatableCompile == RELOCATABLE_COMPILE_OK) {
                // Shared Classes Enabled
                if (!TR::Options::sharedClassCache())
                    ineligibilityReasonForRelocatableCompile = AOT_INELIGIBLE_NO_SCC;
                // If using a loadLimit/loadLimitFile, don't do an AOT compilation
                // for a method body that's already in the SCC
                else if (entry->_methodIsInSharedCache == TR_yes)
                    ineligibilityReasonForRelocatableCompile = AOT_INELIGIBLE_ALREADY_IN_SCC;
                // Eligibility checks
                else if (!fe->sharedCache()->isClassInSharedCache(J9_CLASS_FROM_METHOD(method)))
                    ineligibilityReasonForRelocatableCompile = AOT_INELIGIBLE_CLASS_NOT_IN_SCC;
                // Ensure we can generate a class chain for the class of the
                // method to be compiled
                else if (TR_SharedCache::INVALID_CLASS_CHAIN_OFFSET
                    == fe->sharedCache()->rememberClass(J9_CLASS_FROM_METHOD(method)))
                    ineligibilityReasonForRelocatableCompile = AOT_INELIGIBLE_CLASS_CHAIN;
            }

#if defined(J9VM_OPT_JITSERVER)
            eligibleForRemoteAOTNoSCCCompile
                = (ineligibilityReasonForRelocatableCompileDisregardingSCC == RELOCATABLE_COMPILE_OK)
...
#endif /* defined(J9VM_OPT_JITSERVER) */
            ineligibleForRelocatableCompile = ineligibilityReasonForRelocatableCompile;
        }
        ...
        bool sharedClassTest = (ineligibleForRelocatableCompile == RELOCATABLE_COMPILE_OK)
#if defined(J9VM_OPT_CRIU_SUPPORT)
            && !checkpointInProgress
#endif
        ...
        if (!canDoRelocatableCompile && ineligibleForRelocatableCompile == RELOCATABLE_COMPILE_OK) {
#if defined(J9VM_OPT_CRIU_SUPPORT)
            if (checkpointInProgress)
                ineligibleForRelocatableCompile = AOT_INELIGIBLE_CHECKPOINT_IN_PROGRESS;
            else
#endif
            {
                if (TR::Options::getAOTCmdLineOptions()->getOption(TR_NoStoreAOT))
                    ...
                    ineligibleForRelocatableCompile = AOT_INELIGIBLE_HEURISTIC;
            }
        }
#if defined(J9VM_OPT_JITSERVER)
 ...
 * @param ineligibleForRelocatableCompile Indicates if the current compilation was eligible for AOT (input)
 * @param reloRuntime Pointer to a TR_RelocationRuntime; NULL if JVM does not have AOT support (input)
    ...
    J9::J9SegmentProvider &scratchSegmentProvider)
{
    ...
    AOTIneligibilityReason ineligibleForRelocatableCompile = RELOCATABLE_COMPILE_OK;
    ...
    try {
        ...
        preCompilationTasks(vmThread, entry, method, &aotCachedMethod, trMemory, canDoRelocatableCompile,
            ineligibleForRelocatableCompile, reloRuntime);
        ...
        if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch))
            TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH,
                ...
                "ineligibleForRelocatableCompile=%s isRemoteCompReq=%d _doNotAOTCompile=%d AOTfe=%d isDLT=%d",
                method, entry->isAotLoad(), canDoRelocatableCompile,
                AOTIneligibilityReasonNames[ineligibleForRelocatableCompile], entry->isRemoteCompReq(),
                entry->_doNotAOTCompile, _vm->isAOT_DEPRECATED_DO_NOT_USE(), entry->isDLTCompile());
