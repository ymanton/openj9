      ...
      if (scHints.flags == 0) // If no prior hints exist, we can perform a "storeAttachedData" operation
         ...
         if (!SCfull)
            ...
            if (isFailedValidationHint)
               scHints.count = 10 * _initialHintSCount;
            ...
            if (store == 0)
               {
               if (_verboseHints)
                  TR_VerboseLog::writeLineLocked(TR_Vlog_SCHINTS,"hint added 0x%x, key = %s, scount: %d", scHints.flags, methodSignature, scHints.count);
               }
         ...
         if (updateHint)
            ...
            if (_verboseHints)
               ...
               if (update == 0)
                  {
                  TR_VerboseLog::writeLineLocked(TR_Vlog_SCHINTS,"hint updated 0x%x, key = %s, scount: %d", scHints.flags, methodSignature, scHints.count);
                  }
               else
                  {
